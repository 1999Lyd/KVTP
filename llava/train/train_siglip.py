...
class SiglipDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # get image
        video, frame_time, video_time = self.load_video(item[0], self.max_frames_num, self.fps, force_sample=self.force_sample)
        ...
        return (video, chunked_ids, labels)
...
@dataclass
class DataCollatorForSiglipDataset(object):
    """
    Collate examples for the Siglip dataset without padding or modifications.
    """
    def __call__(self, instances: Sequence[tuple]) -> Dict:
        """
        Collect the data into a dictionary format.

        Args:
            instances (Sequence[Any]): List of individual examples from the dataset.

        Returns:
            Dict[str, Any]: A dictionary containing batched data:
                            - "videos": List of video tensors.
                            - "chunked_ids": List of lists of token chunks.
                            - "labels": List of labels.
        """
        # Extract keys and organize into a dictionary
        videos = [instance[0] for instance in instances]
        chunked_ids = [instance[1] for instance in instances]
        labels = [instance[2] for instance in instances]

        return {
            "videos": videos,             # List of tensors of shape (N_frames, C, H, W)
            "chunked_ids": chunked_ids,   # List of lists of token chunks
...
def make_supervised_data_module_siglip(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    df = pd.read_csv("siglip_train_data.csv")
    train_dataset = SiglipDataset(df=df)
    data_collator = DataCollatorForSiglipDataset()
...
def train(attn_implementation=None):
    ...
    data_module = make_supervised_data_module_siglip(tokenizer=tokenizer, data_args=data_args)
